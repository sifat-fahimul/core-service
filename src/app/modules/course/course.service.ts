...
const getSingleFromDB = async (id: string): Promise<Course | null> => {
  ...
  await prisma.$transaction(async transactionClient => {
    ...
    if (preRequisiteCourses && preRequisiteCourses.length > 0) {
      const deletePrerequisite = preRequisiteCourses.filter(
        coursePrerequisites =>
          coursePrerequisites.courseId && coursePrerequisites.isDeleted
      );
      ...
      await asyncForEach(
        newPrerequisite,
        async (createPreCourse: IPrerequisiteCourseRequest) => {
          await transactionClient.coursetoPrerequisite.create({
            data: {
              courseId: id,
              preRequisiteId: createPreCourse.courseId,
            },
          });
        }
      );
